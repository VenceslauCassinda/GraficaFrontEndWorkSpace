...
import '../../contratos/provedores/provedor_exemplar_i.dart';
import '../../dominio/entidades/Exemplar.dart';
import '../../recursos/constantes.dart';
import '../erros.dart';
import 'package:http_parser/http_parser.dart';

class ProvedorNetExemplar implements ProvedorExemplarI {
  ...
  @override
  Future<int> registarExemplar(Exemplar dado) async {
    int id = -1;
    
    var url = await fazerUploadExemplar(dado.arquivo!.bytes!, dado.arquivo!.name);
    var res = await h.post(
      Uri.parse(URL_ADD_EXEMPLAR),
      headers: {
  ...
  @override
  Future<int> atualizarExemplar(Exemplar dado) async {
    int id = -1;
    var res =
        await h.post(Uri.parse("$URL_ATUALIZAR_EXEMPLAR/${dado.id}/"), headers: {
      "Accept": "aplication/json",
      "Authorization": "Bearer $TOKEN_USUARIO_ATUAL"
    }, body: {
      "id_item": "${dado.idItem}",
  ...
  @override
  Future<List<Exemplar>> pegarListaExemplar() async {
    var lista = <Exemplar>[];
    var res = await h.get(
      Uri.parse(URL_TODOS_EXEMPLAR),
      headers: {
        "Accept": "aplication/json",
        "Authorization": "Bearer $TOKEN_USUARIO_ATUAL"
      },
    ...
    switch (res.statusCode) {
      case 200:
        ...
        for (Map cada in todos) {
          try {
            lista.add(Exemplar.fromJson(cada));
  ...
  @override
  Future<int> removerExemplarDeId(int id) async {
    var res = await h.post(
      Uri.parse("$URL_REMOVER_EXEMPLAR/$id/"),
      headers: {
        "Accept": "aplication/json",
        "Authorization": "Bearer $TOKEN_USUARIO_ATUAL"
      },
